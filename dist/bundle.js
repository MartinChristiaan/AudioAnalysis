...
var NoteState;
(function (NoteState) {
    NoteState[NoteState["ALIVE"] = 0] = "ALIVE";
    NoteState[NoteState["HIT"] = 1] = "HIT";
    NoteState[NoteState["MISS"] = 2] = "MISS";
    NoteState[NoteState["DEAD"] = 3] = "DEAD";
...
class HitDetection {
    ...
    updateKey(key, gear) {
        let curCodes = config_1.keycodes[gear.numNotes];
        if (curCodes.includes(key)) {
            this.activeNotes[curCodes.indexOf(key)] = 5;
        }
    }
    detectHits(visibleNotes, { t_cur }) {
        visibleNotes.forEach(note => {
            var t_min = note.t_onset - config_1.margin;
            var t_max = note.t_onset + config_1.margin;
...
document.addEventListener('keydown', function (event) {
    ...
    if (event.code == 'Space') {
        ...
        main_1.dynamicsManager.applyBooster();
    }
    else if (key == "Shift") {
        shiftDown = true;
    ...
    if (shiftDown == false) {
        main_1.hitDetection.updateKey(key, main_1.gearbox.curGear);
    }
    else {
        let shiftKeyCodes = config_1.keycodes[4].map(x => x.toUpperCase());
        if (shiftKeyCodes.includes(key)) {
            // console.log("Shifting to gear " + shiftKeyCodes.indexOf(key))
            main_1.gearbox.shiftGear(shiftKeyCodes.indexOf(key));
            main_1.noteDrawer.shiftGear(main_1.gearbox.curGear, main_1.song);
        }
    }
});
...
},{"./config":4,"./main":11}],9:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
...
},{"./postprocessing":14}],10:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
...
exports.song = new song_1.Song(songdata);
exports.dynamicsManager = new dynamicsManager_1.DynamicsManager(exports.song, postprocessing_1.screenShaker);
exports.noteColorer = new colors_1.NoteColorer(exports.dynamicsManager);
exports.hitDetection = new hitDetection_1.HitDetection(exports.dynamicsManager);
exports.gearbox = new gearmanager_1.GearBox(songdata);
exports.noteDrawer = new noteDrawer_1.NoteDrawer();
exports.instrumentDrawer = new instrumentDrawer_1.InstrumentDrawer(exports.song, exports.ctx, exports.noteColorer);
exports.scoreDrawer = new ScoreDrawer_1.ScoreDrawer(exports.ctx, exports.dynamicsManager, exports.song);
console.timeEnd('Initialization');
function main() {
    exports.ctx.clearRect(0, 0, innerWidth, innerHeight); // clear canvas
    exports.dynamicsManager.update();
    var speed = exports.dynamicsManager.speed;
    exports.song.update(speed);
    exports.noteDrawer.update(exports.gearbox.curGear, exports.song);
    exports.hitDetection.detectHits(exports.noteDrawer.visibleNotes, exports.song);
    exports.instrumentDrawer.update(exports.hitDetection.activeNotes, exports.gearbox.curGear);
    exports.scoreDrawer.drawScore();
    postprocessing_1.screenShaker.update();
    window.requestAnimationFrame(main);
}
main();
console.log(inputHandler_1.load);
// TODO - 
// Add boosters
// Special notes 

},{"./ScoreDrawer":2,"./colors":3,"./dynamicsManager":5,"./gearmanager":6,"./hitDetection":7,"./inputHandler":8,"./instrumentDrawer":9,"./loader":10,"./noteDrawer":12,"./postprocessing":14,"./song":16}],12:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
    ...
    update(gear, song) {
        song.visibleIdx.forEach(idx => {
            if (idx > this.spawnedIDX) {
                this.spawnedIDX = idx;
                this.visibleNotes.push(new noteParticle_1.Note(song, gear, idx));
            }
        });
        this.visibleNotes.forEach(note => { note.update(); });
        this.visibleNotes = this.visibleNotes.filter(note => note.state != hitDetection_1.NoteState.DEAD);
    }
}
exports.NoteDrawer = NoteDrawer;

},{"./hitDetection":7,"./noteParticle":13}],13:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const hitDetection_1 = require("./hitDetection");
const config_1 = require("./config");
...
class Note {
    constructor(song, gear, idx) {
        this.t_onset = song.t_onset[idx]; // load from analyis  
        let myf_onset = song.f_onset[idx];
        this.state = hitDetection_1.NoteState.ALIVE;
        this.song = song;
        this.y = innerHeight;
        this.alpha = 1;
        this.TrailParticles = [];
        this.color = main_1.noteColorer.getNoteColorRaw(this.t);
        this.livetimeAfterDead = 20;
        this.orbitParticles = [];
        this.idx = idx;
        this.changeGear(song, gear);
    }
    changeGear({ f_onset, maxFreq }, { numNotes, downSample }) {
        if (this.idx % downSample !== 0) {
            this.state = hitDetection_1.NoteState.HIT;
        }
    ...
    update() {
        ...
        if (this.state == hitDetection_1.NoteState.ALIVE) {
            if (Math.random() < 0.25) {
                this.TrailParticles.push(new TrailParticle(this.x, this.y, innerHeight / (this.song.t_max - this.song.t_min), colors_1.addRandomDeviation(this.color, 20).getHTMLValue()));
            }
        }
    ...
    draw() {
        ...
        if (state == hitDetection_1.NoteState.ALIVE) {
            this.orbitParticles.forEach(x => { x.update(this.x, this.y); });
        }
        else if (state == hitDetection_1.NoteState.HIT) {
            this.orbitParticles.forEach(x => { x.updateExplode(); });
            this.livetimeAfterDead -= 1;
            if (this.livetimeAfterDead == 0) {
                this.state = hitDetection_1.NoteState.DEAD;
            }
        }
        else if (state == hitDetection_1.NoteState.MISS) {
            main_1.ctx.beginPath();
            main_1.ctx.fillStyle = "red";
...
},{"./colors":3,"./config":4,"./hitDetection":7,"./main":11,"./postprocessing":14}],14:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
...
},{"./main":11,"./screenshake":15}],15:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
class ScreenShaker {
    constructor() {
...
},{"./config":4,"howler":1}]},{},[11])
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
